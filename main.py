...
"""Calculate spread (gap) between sell and buy for 24k"""
try:
if "الذهب عيار 24" in data and isinstance(data["الذهب عيار 24"], dict):
sell = float(data["الذهب عيار 24"]["sell"])
buy = float(data["الذهب عيار 24"]["buy"])
spread = sell - buy
return f"{int(spread)}"
...
display_data["الفجوة السعرية"] = spread

# Ensure الدولار الأمريكي exists (from backup source or use placeholder)
if "الدولار الأمريكي" not in display_data:
display_data["الدولار الأمريكي"] = "--"

return format_prices("💎 <b>تحديث لحظي للذهب</b>", display_data)
...
for karat in KARAT_ORDER:
if karat in display_data and isinstance(display_data[karat], dict):
v = display_data[karat]
msg += f"🔸 <b>{karat}:</b>\n"
msg += f"🟢 بيع: {format_price(v['sell'])} | 🔴 شراء: {format_price(v['buy'])}\n"
...
msg += f"📌 {key}: <b>{display_data[key]}</b>\n"
...
html += &lt;div class="card"&gt; &lt;div class="karat-title"&gt;🔸 ${karat}</div>
<div class="prices-row">
<div class="price-box sell">
<div class="price-label">🟢 سعر البيع</div>
<div class="price-value">${formatPrice(data[karat].sell)}&lt;/div&gt; &lt;/div&gt; &lt;div class="price-box buy"&gt; &lt;div class="price-label"&gt;🔴 سعر الشراء&lt;/div&gt; &lt;div class="price-value"&gt;${formatPrice(data[karat].buy)}</div>
</div>
</div>
</div>
`;
...
extrasHtml += &lt;div class="extra-item"&gt; &lt;span class="extra-label"&gt;📌${item.label}</span>
<span class="extra-value">${value}&lt;/span&gt; &lt;/div&gt;;
});
