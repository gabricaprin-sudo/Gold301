...
def normalize_for_hash(data):
    """تطبيع البيانات لحساب الـ Hash بشكل متسق"""
    gold_only = {}
    for k, v in data.items():
        if isinstance(v, dict):
            gold_only[k] = {"buy": str(v["buy"]), "sell": str(v["sell"])}
    return json.dumps(gold_only, sort_keys=True, ensure_ascii=False)
...
def get_snapshot(retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            # حساب الـ Hash باستخدام التطبيع المتسق
...
# =====================
def loop():
    global last_hash, last_data, sent_close_msg, sent_open_msg, yesterday_close, loop_running

    with loop_lock:
        if loop_running:
            log.warning("Loop already running, skipping")
            return
        loop_running = True
